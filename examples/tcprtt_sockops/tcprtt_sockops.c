...
static inline void init_sk_key(struct bpf_sock_ops *skops,
                               struct sk_key *sk_key) {
...
static inline void bpf_sock_ops_establish_cb(struct bpf_sock_ops *skops,
  ...
  err = bpf_map_update_elem(&map_estab_sk, &sk_info.sk_key, &sk_info,
                            BPF_NOEXIST);
  if (err != 0) {
    // Storing the 4-tuple in map has failed, return early.
    // This can happen in case the 4-tuple already exists in the map (i.e.
    // BPF_NOEXIST flag)
    return;
  }
